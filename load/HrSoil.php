			...
			?>
				...
				$arrayData = array();  	
					$query ="SELECT DISTINCT(data) FROM hrsoil_mean_h1 WHERE data BETWEEN ".$da." AND ".$a." ORDER BY data";
					...
					for ($contatore=0; $contatore < $maxN; $contatore++) {
						...
						$query ="SELECT data,tseries FROM hrsoil_mean_h1 WHERE idstazione = ".$stazioni[$contatore]. " AND data BETWEEN ".$da." AND ".$a." ORDER BY data";
					...
					var charthrsoil = new Highcharts.Chart(options);
					</script>
					<?php
					}
					else{?>
					...
					charthrsoil.addSeries({name: jArrayNomeStazione[0],data:d2,turboThreshold: 0});
					</script>
					<?php
					}
			}}