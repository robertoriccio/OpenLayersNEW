			...
			?>
				...
				$arrayData = array();  	
					$query ="SELECT DISTINCT(data) FROM hum_min_h24 WHERE data BETWEEN ".$da." AND ".$a." ORDER BY data";
					...
					for ($contatore=0; $contatore < $maxN; $contatore++) {
						...
						$query ="SELECT data,tseries FROM hum_min_h24 WHERE idstazione = ".$stazioni[$contatore]. " AND data BETWEEN ".$da." AND ".$a." ORDER BY data";
					...
					charthum.addSeries({name: jArrayNomeStazione[0],data:d2,turboThreshold: 0});
					</script>
					<?php
					}
			}}