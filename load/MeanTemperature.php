		...
		if($variabili[3] <> 'no'){
				...
				$arrayData = array();  	
										  if($variabili[3] == 'meantemph24'){$query ="SELECT DISTINCT(data) FROM temp_mean_h24 WHERE data BETWEEN ".$da." AND ".$a." ORDER BY data";}
					  else{$query ="SELECT DISTINCT(data) FROM temp_mean_h1 WHERE data BETWEEN ".$da." AND ".$a." ORDER BY data";}
					...
					for ($contatore=0; $contatore < $maxN; $contatore++) {
						...
						if($variabili[3] == 'meantemph24'){$query ="SELECT data,tseries FROM temp_mean_h24 WHERE idstazione = ".$stazioni[$contatore]. " AND data BETWEEN ".$da." AND ".$a." ORDER BY data";}
						else{$query ="SELECT data,tseries FROM temp_mean_h1 WHERE idstazione = ".$stazioni[$contatore]. " AND data BETWEEN ".$da." AND ".$a." ORDER BY data";}
