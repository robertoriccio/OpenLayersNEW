			...
			?>
				...
				$arrayData = array();  	
					$query ="SELECT DISTINCT(data) FROM tsoil_mean_h1 WHERE data BETWEEN ".$da." AND ".$a." ORDER BY data";
					...
					for ($contatore=0; $contatore < $maxN; $contatore++) {
						...
						$query ="SELECT data,tseries FROM tsoil_mean_h1 WHERE idstazione = ".$stazioni[$contatore]. " AND data BETWEEN ".$da." AND ".$a." ORDER BY data";
					...
					var charttsoil = new Highcharts.Chart(options);
					</script>
					<?php
					}
					else{?>
					...
					charttsoil.addSeries({name: jArrayNomeStazione[0],data:d2,turboThreshold: 0});
					</script>
					<?php
					}
			}}