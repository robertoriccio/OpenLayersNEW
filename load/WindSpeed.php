			...
			?>
				...
				$arrayData = array();  	
					$query ="SELECT DISTINCT(data) FROM wind_speed_h1 WHERE data BETWEEN ".$da." AND ".$a." ORDER BY data";
					...
					for ($contatore=0; $contatore < $maxN; $contatore++) {
						...
						$query ="SELECT data,tseries FROM wind_speed_h1 WHERE idstazione = ".$stazioni[$contatore]. " AND data BETWEEN ".$da." AND ".$a." ORDER BY data";
					...
					var chartwind = new Highcharts.Chart(options);
					</script>
					<?php
					}
					else{?>
					...
					chartwind.addSeries({name: jArrayNomeStazione[0],data:d2,turboThreshold: 0});
					</script>
					<?php
					}
			}}